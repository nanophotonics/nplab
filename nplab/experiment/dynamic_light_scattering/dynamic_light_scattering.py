    ...
    def __init__(self, instruments):
        ...
        if "sample_rotation_stage" in list(instruments.keys()):
            ...
            self.textboxes.update(
                {"new_angle": self.sample_rotation_stage_ui.new_angle_textbox})
            self.textboxes.update({
                "rotation_speed":
                self.sample_rotation_stage_ui.rotation_speed_textbox
            })
            self.fields.update(
                {"zero_angle": self.sample_rotation_stage.zero_pos})
        ...
        assert (len(list(set(checkbox_keys).intersection(
            set(textbox_keys)))) == 0)
        assert (len(list(set(checkbox_keys).intersection(
            set(field_keys)))) == 0)
        assert (len(list(set(field_keys).intersection(
            set(textbox_keys)))) == 0)
    ...
    def run_experiment(self, path, debug=False):
        ...
        for i, settings in enumerate(steps):
            ...
            self.adc_ui.threaded_capture(
                settings={"experiment_settings": settings})
...
class DynamicLightScatteringUI(QtWidgets.QWidget, UiTools):
    def __init__(self, experiment, parent=None, debug=False, verbose=False):
        if not isinstance(experiment, DynamicLightScattering):
            raise ValueError(
                "Object is not an instance of the DynamicLightScattering")
        super(DynamicLightScatteringUI, self).__init__()
        self.experiment = experiment
        uic.loadUi(
            os.path.join(os.path.dirname(__file__),
                         'dynamic_light_scattering_experiment.ui'), self)
    ...
    def run_experiment(self):
        try:
            t = threading.Thread(target=self.experiment.run_experiment,
                                 args=(self.run_path, ))
            t.start()
        except Exception as e:
            self.experiment.log(
                "Error when running experiment - have you written the path?",
                level="error")
...
config_loader = DynamicLightScattering(
    instruments={"adc":
                 adc})  ##,"sample_rotation_stage":sample_rotation_stage})
instruments = {
    "adlink9812": adc,
    "config": config_loader
}  #, "stage":sample_rotation_stage}
...
gui = GuiGenerator(instrument_dict=instruments,
                   dock_settings_path=os.path.join(os.path.dirname(__file__),
                                                   "experiment_ui.npy"),
                   scripts_path=None,
                   working_directory="~")
app.exec_()
