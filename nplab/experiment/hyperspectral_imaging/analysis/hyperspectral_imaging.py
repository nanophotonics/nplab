...
class HyperspectralImage(object):
    ...
    def _load_data(self, scan):
        ...
        if 'num_spectrometers' not in self.attrs:
            self.attrs['num_spectrometers'] = len(
                [s for s in list(scan.keys()) if 'spectra' in s])
    ...
    def wavelength_lims(self, value):
        ...
        self._wavelength_roi = get_roi(self.scan['wavelength'][()], value[0],
                                       value[1])
    ...
    def wavelength2_lims(self, value):
        ...
        if 'wavelength2' in self.scan:
            ...
            self._wavelength2_roi = get_roi(self.scan['wavelength2'][()],
                                            value[0], value[1])
        else:
    ...
    def spectra(self):
        ...
        if ndim == 2:
            ...
            roi = np.s_[self._z_roi, self._y_roi, self._x_roi,
                        self._wavelength_roi]
    ...
    def _load_calibrations(self):
        ...
        self._dx1, self._dy1 = self._get_offset(self.wavelength,
                                                polarisation=1)
        self._dx2, self._dy2 = self._get_offset(self.wavelength,
                                                polarisation=2)
    ...
    def get_spectra(self, polarisation=1):
        if polarisation == 2:
            assert self.attrs[
                'num_spectrometers'] == 2, 'polarisation does not exist'
    ...
    def integrate_spectra(self,
        ...
        spectra = spectra[axslice, y_roi,
                          x_roi, :] if spectra.ndim > 3 else spectra[y_roi,
                                                                     x_roi, :]
    ...
    def reconstruct_colour_image(self,
                                 polarisation=1,
                                 norm=True,
                                 axslice=None):
    ...
    def create_calibration(self,
        ...
        for i, wl in enumerate(wavelength):
            ...
            x0s[i] = x0
            y0s[i] = y0
        roi = slice(np.min(np.where(500 < wavelength)),
                    np.max(np.where(wavelength < 1000)))
    ...
    def _get_offset(self, wavelength, polarisation=1, unit='SI'):
        ...
        if unit == 'SI':
            ...
            dpx = [abs(self.x - i).argmin()
                   for i in xo] - abs(self.x - xr).argmin()
            dpy = [abs(self.y - i).argmin()
                   for i in yo] - abs(self.y - yr).argmin()
            return dpx, dpy
    ...
    def _correct_chromatic_aberration(self, polarisation=1, axslice=None):
        ...
        for i in range(s[-1]):  # for each wavelength (last dimension)
            ...
            if spectra.ndim > 3:
                big_img[s11 - dx:s12 - dx, s21 - dy:s22 - dy, :,
                        i] = spectra[:, :, :, i]
            else:
                big_img[s11 - dx:s12 - dx, s21 - dy:s22 - dy,
                        i] = spectra[:, :, i]
...
if __name__ == '__main__':
    hsimg = HyperspectralImage(
        '/Users/alansanders/Desktop/Data/2014/08. Aug/26/data.hdf5',
