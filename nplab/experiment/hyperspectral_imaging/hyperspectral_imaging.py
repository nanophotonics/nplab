...
class HyperspectralScan(GridScanQt, ScanningExperimentHDF5):
    ...
    def set_spectrometers(self, spectrometers):
        if not isinstance(spectrometers, (Spectrometer, Spectrometers)):
            raise ValueError(
                'spectrometer must be an instance of either Spectrometer or '
                'Spectrometers')
    ...
    def open_scan(self):
        ...
        self.data = group.create_group(
            'scan_%d', attrs=dict(description=self.description))
        ...
        for i in range(self.num_spectrometers):
            ...
            self.data.create_dataset('wavelength' + suffix,
                                     data=spectrometer.wavelengths)
            self.data.create_dataset('hs_image' + suffix,
                                     shape=self.grid_shape +
                                     (spectrometer.wavelengths.size, ),
                                     dtype=np.float64,
    ...
    def set_latest_view(self, *indices):
        ...
        for i in range(self.num_spectrometers):
            ...
            if self.num_axes == 2:
                latest_view = data[:, :, w]
                spectrum = self.data['hs_image' + suffix][indices[-2],
                                                          indices[-1], :]
                ...
                spectrum = self.data['hs_image' + suffix][k, indices[-2],
                                                          indices[-1], :]
    ...
    def estimated_step_time(self):
        if isinstance(self.spectrometer, Spectrometer):
            ...
            max_exposure = 1e-3 * max(
                [s.integration_time for s in self.spectrometer.spectrometers])
        else:
            max_exposure = 0
            warnings.warn(
...
class HyperspectralScanUI(QtWidgets.QWidget, UiTools):
    ...
    def init_view_wavelength_controls(self):
        ...
        self.view_wavelength.returnPressed.connect(
            self.on_view_wavelength_change)
        ...
        self.wavelength_range.valueChanged[int].connect(
            self.on_wavelength_range_change)
    ...
    def init_view_select(self):
        ...
        self.view_layer.textChanged.connect(
            self.on_view_layer_change
        )  # (partial(setattr, self.grid_scanner, 'view_layer'))
        self.grid_scanner.view_layer_updated.connect(
            self.on_gs_view_layer_change)

        self.override_view_layer.stateChanged.connect(
            self.on_override_view_layer)

    def update_param(self, *args, **kwargs):
        sender = self.sender()
        if sender == self.scan_description:
            self.grid_scanner.description = self.scan_description.toPlainText(
            ).encode('utf8')
