    ...
    * in your ``run()`` method, call ``self.update_progress(i)`` periodically.
    ...
    def prepare_to_run(self, *args, **kwargs):
        ...
        raise NotImplementedError(
            "Experiments with progress bars must set self.progress_maximum"
    ...
    def run_modally(self, *args, **kwargs):
        ...
        if self.progress_maximum is None:
            raise NotImplementedError(
                "self.progress_maximum was not set - this is necessary.")
        self._progress_bar = QProgressDialogWithDeferredUpdate(
            self.__class__.__name__, "Abort", self.progress_minimum,
...
#     self._progress_bar.exec_()
    ...
    def update_progress(self, progress):
        ...
        try:
            ...
            print(
                "Error setting progress bar to {} (are you running via run_modally()?)"
                .format(progress))
...
class RunFunctionWithProgressBar(ExperimentWithProgressBar):
    ...
    def __init__(self, target, progress_maximum=None, *args, **kwargs):
        ...
        self.target = target
        assert callable(target), ValueError(
            "The target function is not callable!")
    ...
    def run(self, *args, **kwargs):
        ...
        self.update_progress(
            self.progress_maximum
        )  # Ensure the progress dialog closes unless we're aborted.
