...
class Pacton(Instrument):
    ...
    def get_image(self, center_wavelength, roi=None, debug=0):
        self.acton.set_wavelength(center_wavelength,
                                  blocking=True,
                                  debug=debug)
        if roi is not None:
    ...
    def pixel_to_wavelength(self, pixels, center_wavelength, intensity):
        ...
        if self.pixel_to_wl_sf is None:
            raise ValueError(
                "No conversion factor set, please run Pacton.calibrate routine"
            )
        else:
            #intensity - the counts on each pixel
            wavelengths = center_wavelength + self.pixel_to_wl_sf * np.array(
                pixels)
            min_wl = np.min(wavelengths)
            max_wl = np.max(wavelengths)

            indices = np.logical_and(CALIBRATION_WAVELENGTHS >= min_wl,
                                     CALIBRATION_WAVELENGTHS < max_wl)

            calibration_intensity = CALIBRATION_COUNTS[indices]
            calibration_wavelengths = CALIBRATION_WAVELENGTHS[indices]

            calibration_intensity = calibration_intensity - np.min(
                calibration_intensity)
            calibration_intensity = old_div(calibration_intensity,
                                            np.max(calibration_intensity))

            intensity = intensity - np.min(intensity)
            intensity = old_div(intensity, np.max(intensity))

            upsampled_calibration_intensity = scipy.signal.resample(
                calibration_intensity,
                len(wavelengths),
                t=None,
                axis=0,
                window=None)
            upsampled_calibration_wavelengths = scipy.signal.resample(
            ...
            max_xcs, xcs = spectrum_pixel_offset(
                upsampled_calibration_intensity, intensity)
            xcs = np.fft.fftshift(xcs)
            ...
            # 	ax1.legend()
            # 	plt.show()
            wavelengths = center_wavelength + self.pixel_to_wl_sf * (
                np.array(pixels) + offset)
    ...
    def get_spectrum(self,
        ...
        wavelengths = self.pixel_to_wavelength(pixel_offsets,
                                               center_wavelength, spectrum)
    ...
    def calibrate(self,
            ...
            for j in range(i + 1, len(spectra)):
                ...
                if plot == True:
                    p = axarr[1].plot(
                        ...
                        "-",
                        label=
                        "spectra: {0}nm vs {1}nm [offset: {2:.3g}, scale factor: {3:.3g}]"
                        .format(wavelengths[i], wavelengths[j], peak, sf))
                    axarr[1].plot(peak, xcs[peak], "o", color=p[0].get_color())

                if debug > 0:
                    print(
                        "Scale factor [{0} nm] vs [{1} nm]:".format(
                            wavelengths[i], wavelengths[j]), sf)
        ...
        if debug > 0:
            print(
                "Final pixel to wavelength conversion  factor: {0:4g} +/- {1:4g} [nm/pixel]"
                .format(sf_mean, sf_std))
