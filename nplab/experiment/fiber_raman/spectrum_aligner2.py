...
def merge_spectra(ys0, ys1, shift):
    ...
    if (relative_shift + ys1.shape[1]) - ys0.shape[1] > 0:
        ys0_pad = np.zeros(
            (ys0.shape[0], (relative_shift + ys1.shape[1]) - ys0.shape[1]))
...
def load_reference_data(reference_file,
    ...
    if debug > 0:
        ...
        ax3.set_title(
            "Reference spectrum,\n Wavelength lower bound:{0}\n Wavelength upper bound:{1}"
            .format(lower_wavelength, upper_wavelength))
...
def make_data_matrix(data):
    ...
    for i in range(1, len(data)):
        ys1 = [data[i][2]]
        data_matrix, relative_shift = merge_spectra(data_matrix, ys1,
                                                    ys_shift[i])
...
def median_spectrum(data, debug=0):
    ...
    if debug > 0:
        ...
        spectrum = apply_function(
            data_matrix, [np.median, np.mean, np.std, max_fun, min_fun, len])
    ...
    if debug > 0:
        ...
        ax.plot(wavelengths,
                [pixel_to_index(wl, lower_bound) for wl in wavelengths],
                label="Leftmost pixel index (0) (real pixel value: 10)")
        ax.plot(wavelengths,
                [pixel_to_index(wl, upper_bound) for wl in wavelengths],
...
def rescale_reference(xs, ys, max_size, N, debug=0):
    ...
    if debug > 0:
        ...
        ax.plot(xs)
        plt.plot(list(range(0, len(xs))),
                 [offset + gradient * x for x in range(0, len(xs))],
...
def prune_peaks(peaks):
    ...
    for i in range(peaks.shape[1]):
        ...
        print(peaks)
        diff = [
            np.linalg.norm(peaks[:, j].T - v) for j in range(peaks.shape[1])
        ]
        diff[i] = np.inf
...
def remove_indices(peaks, indices):
    peaks = np.array(
        [peaks[:, i] for i in range(peaks.shape[1]) if i not in indices]).T
    return peaks
...
def link_peaks(signal_peaks,
    ...
    reference_peaks = remove_indices(reference_peaks,
                                     ignored_reference_indices)
