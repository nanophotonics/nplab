...
def software_lockin(t,
    ...
    :rtype : object
    """
    assert len(signal) == len(t) and len(reference) == len(
        t), 'all arrays must be the same length.'
    ...
    zero_crossings = t[cond1
                       & cond2]  # these are the points in time that are zero
    ...
    ref = np.exp(-1j * harmonic *
                 (omega_r * t + phi_r))  # construct the reference waveform
