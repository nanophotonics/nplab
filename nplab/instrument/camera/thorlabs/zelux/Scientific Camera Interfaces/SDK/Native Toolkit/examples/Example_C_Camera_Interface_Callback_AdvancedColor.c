...
pthread_cond_t frame_acquired_event;
#endif
...
void camera_connect_callback(char* camera_serial_number, enum TL_CAMERA_USB_PORT_TYPE usb_bus_speed, void* context);
...
int main(void)
{
	...
	if (pthread_mutex_init(&lock, NULL))
		return report_error_and_cleanup_resources("Unable to initialize pthread mutex lock");
	if (pthread_cond_init(&frame_acquired_event, NULL))
		return report_error_and_cleanup_resources("Unable to initialize pthread condition variable frame_acquired_event");
#endif
