...
int main(void)
{
	...
	*/
	...
	output_buffer = malloc(sizeof(unsigned short) * image_width * image_height * 3);
	...
	while (count < 10)
	{
		...
		// transform monochrome image to 48 bpp color image
		if (tl_mono_to_color_transform_to_48(mono_to_color_processor_handle, image_buffer, image_width, image_height, output_buffer))
			return report_error_and_cleanup_resources(tl_mono_to_color_get_last_error());
