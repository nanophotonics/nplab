...
class CameraWithLocation(Instrument):
    ...
    def __init__(self, camera=None, stage=None, init_exp = None):
  ...
  #                                                                               "as when we calibrated!" #These lines dont make much sense, the iwl has the size of the image while the martix is always only 3x3
    ...
    def move_to_feature(self, feature, ignore_position=False, ignore_z_pos = False, margin=50, init_exp = 200, tolerance=0.5, max_iterations = 10, p_number = None):
        ...
        """
        if p_number != None:
            self.log('Switching to particle %s' % p_number)
        ...
        image = self.color_image()#Image of current camera view
        ...
        for i in range(max_iterations):
            ...
            try:
                ...
                print 'Feature located'
                
                #except Exception as e1:
                #self.log('Feature could not be located in image because %s' % e1)
                    
                new_position = image.pixel_to_location(pixel_position)
                self.move(new_position)
                print 'Moved to feature'
                ...
                self.log("Error centering on feature, iteration {} raised an exception:\n{}\n".format(i, e2) +
                         ...
                         "The image size was {}\n".format(image.shape))
                self.log('Returning camera to desired exposure setting')
                self.camera.exposure = init_exp
    ...
    def autofocus(self, dz=None, merit_function=af_merit_squared_laplacian,
                  ...
                  use_thumbnail = False, exposure_iterations = 3, exposure_step = 200, update_progress=lambda p:p):
        ...
        """
        initial_exposure = self.camera.exposure        
        for attempt in range(exposure_iterations):
            print 'Exposure = %s' % self.camera.exposure
            ...
            if method == "centre_of_mass":
                ...
                if (np.sum(weights) == 0):
                    ...
                    if attempt < exposure_iterations - 1:
                        print("Warning, a maximum autofocus score could not be found. Returning to initial position and trying again with longer exposure.")
                    else:
                        'Autofocus failed'
                    new_position = here
                    self.camera.exposure *= exposure_factor
                    self.camera.exposure += exposure_step#Make image brighter and try again
            ...
            self.camera.exposure = initial_exposure
            print 'Returning to inital exposure setting of %s' % self.camera.exposure
            break
            ...
            for y_index in y_indices:
                for x_index in x_indices:
                    ...
                    if self.autofocus == True:
                        afSteps = 5 #Slightly quicker autofocus
                        afStepSize = 1
                        afDz = (np.arange(afSteps) - (afSteps - 1)/2) * afStepSize
                        cwl.autofocus(dz = afDz)
