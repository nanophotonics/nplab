...
Calibration_Arrays[2].append(
    [764.61, 2.27389453e-07, -1.50626785e-01, 1.21487041e+04])
Calibration_Arrays[2].append(
    [789.48, 1.59023464e-07, -1.38884759e-01, 1.19635203e+04])
Calibration_Arrays[2].append(
    [823.5, 4.03268378e-08, -1.17709195e-01, 1.14389462e+04])
Calibration_Arrays[2].append(
    [828.15, 1.2025391e-08, -1.1258273e-01, 1.1259733e+04])
Calibration_Arrays[2].append(
    [834.76, -1.61822540e-08, -1.07392158e-01, 1.11077126e+04])
Calibration_Arrays[2].append(
    [841.12, -6.09192196e-08, -9.91452193e-02, 1.08073451e+04])
Calibration_Arrays[2].append(
    [882.44, -2.41269681e-07, -6.46639473e-02, 9.68027416e+03])
Calibration_Arrays[2].append(
    [895.32, -2.42364194e-07, -6.43223220e-02, 9.82060231e+03])
Calibration_Arrays[2].append(
    [904.72, -2.15463477e-07, -6.95790273e-02, 1.01941429e+04])
Calibration_Arrays[2].append(
    [916.35, -1.62169936e-07, -8.02752427e-02, 1.08757910e+04])
Calibration_Arrays[2].append(
    [938.31, -4.38217497e-09, -1.12728352e-01, 1.28287277e+04])
Calibration_Arrays[2].append(
    [979.72, 2.16188951e-07, -1.59901057e-01, 1.58551361e+04])
Calibration_Arrays[2].append(
    [992.11, 2.23593080e-07, -1.61622854e-01, 1.61153277e+04])
...
def Quad_Interp(x_Points, y_Points, Input):  #x are in order
    ...
    while Min <= Max:
        ...
        if np.sum(Mask) != 0:
            ...
            if Min - 2 >= 0:
                Poly.append(
                    np.polyfit(x_Points[Min - 2:Min + 1],
                               y_Points[Min - 2:Min + 1], 2))
            if Min + 1 < len(y_Points):
                Poly.append(
                    np.polyfit(x_Points[Min - 1:Min + 2],
                               y_Points[Min - 1:Min + 2], 2))
            if len(Poly) == 1:
                Output[Mask] = np.polyval(Poly[0], Input[Mask])
            else:
                for i in range(2):
                    Poly[i] = np.polyval(Poly[i], Input[Mask])
                Frac = (Input[Mask] - x_Points[Min - 1]) / (x_Points[Min] -
                                                            x_Points[Min - 1])
...
class Triax(VisaInstrument):
    ...
    def Grating(self, Set_To=None):
        ...
        if Set_To not in [None, 0, 1, 2]:
            raise ValueError(
                'Invalid input for grating input. Must be None, 0, 1, or 2.')
    ...
    def Convert_Pixels_to_Wavelengths(self, Pixel_Array, Steps=None):
        ...
        for i in Calibration_Data:
            ...
            Extremes.append([
                ...
                (-b + Root *
                 ((b**2 - (4 * a *
                           (c - self.Number_of_Pixels + 1)))**0.5)) / (2 * a)
            ])
        ...
        Output = Quad_Interp(np.array(Known_Pixels), np.array(Wavelengths),
    ...
    def Find_Required_Step(self, Wavelength, Pixel, Require_Integer=True):
        ...
        for i in Bounds:
            Values.append(
                self.Convert_Pixels_to_Wavelengths([Pixel], Steps=i)[0])
...
class TriaxUI(QtWidgets.QWidget, UiTools):
    ...
    def set_wl_gui(self):
        self.triax.Set_Center_Wavelength(
            float(self.centre_wl_lineEdit.text().strip()))
...
class Trandor(Andor):  #Andor
    ...
    def Generate_Wavelength_Axis(self, use_shifts=None):
        ...
        if use_shifts:
            ...
            return (1. / (self.laser_wavelength * 1e-9) - 1. /
                    (wavelengths * 1e-9)) / 100
        else:
