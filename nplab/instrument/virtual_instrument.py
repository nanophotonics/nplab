...
class VirtualInstrument_listener(object):
    ...
    essentially creates a memory map and waits for commands. Upon recieving a command the instrument 
    ...
    Args:
        ...
        memory_indentifier(str): The memory str identifier - this is usually set as the "VirtualInstMemory_'classname'"
        '''
    def __init__(self,memory_size=65536,memory_identifier='VirtualInstMemory'):
        ...
        np.set_printoptions(threshold=np.inf) #Set the prints options so that the arrays are printed as strings with no shortening
