...
class Raman_Spectrum(object):
    ...
    def __init__(self, filename, timestamp, metadata, laserWl, laserPower,
                 absLaserPower, integrationTime, accumulations, nScans,
...
def extractRamanSpc(path, bg_path=False, combine_statics=False):
    ...
    for n, spcFile in enumerate(spcFiles):
        ...
        fDicts = [f.__dict__,
                  f.log_dict]  #main dictionaries containing spectral metadata
        ...
        ]:  #the timestamp values are actually arbitrary, so this is obsolete
            if unit == 'year':
                zFill = 4
            else:
                zFill = 2
        ...
        accumulations = int(
            fLogDict['Accumulations'].split(': ')[-1])  #number of scans
        ...
        if absLaserPower != 'Undefined':
            ...
            absRamanIntensities = np.array([
                spectrum * 1000 /
                (absLaserPower * integrationTime * float(accumulations / 1000))
        ...
        spectra.append(
            Raman_Spectrum(filename, timestamp, metadata, laserWl, laserPower,
...
def populateH5(spectra, h5File):
    ...
    with h5py.File(h5File, 'a') as f:
        ...
        for n, spectrum in enumerate(spectra):
            ...
            dAbs = gSpectrum.create_dataset('Raman (cts mw^-1 s^-1)',
                                            data=np.array(yAbs))
        ...
        for spectrumName in spectraNames:
            ...
            dAbs.attrs.update(
                f['Spectra'][spectrumName]['Raman (cts mw^-1 s^-1)'].attrs)
            ...
            dNorm.attrs.update(
                f['Spectra'][spectrumName]['Raman (normalised)'].attrs)
