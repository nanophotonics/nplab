__author__ = 'alansanders', 'eoinelliott'
import re 
...
# from .background_removal.asymmetric_least_squares import als
import nplab.analysis.background_removal.asymmetric_least_squares as ALS
...
class Spectrum(np.ndarray):
    ...
    def als(self, **kwargs):
        return self.__class__(self - ALS.als(self, **kwargs), self.x)
...
class RamanSpectrum(Spectrum):
    ...
    def __array_finalize__(self, obj):
        ...
        if not obj.shape:
            return np.array(obj)
        self._wavelengths = getattr(obj,
                                   '_wavelengths',
                                   None)
        self._shifts = getattr(obj, '_shifts', None)
        self.laser_wavelength = getattr(obj, 'laser_wavelength', None)
    
    def __reduce__(self):
        # Get the parent's __reduce__ tuple
        pickled_state = super().__reduce__()
        # Create our own tuple to pass to __setstate__
        new_state = pickled_state[2] + (self.laser_wavelength, self._wavelengths, self._shifts)
        # Return a tuple that replaces the parent's __setstate__ tuple with our own
        return (pickled_state[0], pickled_state[1], new_state)

    def __setstate__(self, state):
        self._shifts = state[-1]  # Set the info attribute
        self._wavelengths = state[-2]
        self.laser_wavelength = state[-3]
        # Call the parent's __setstate__ with the other tuple elements.
    ...
    def shifts(self):
        if self._shifts is None:
            return (1./(self.laser_wavelength*1e-9) - 1./(self._wavelengths*1e-9))/100.
        return self._shifts
    
    @cached_property # only ever calculated once per instance
    def wavelengths(self):
        if self._wavelengths is None:
            return 1e7/(((1/ (self.laser_wavelength * 1e-9) - self.shifts * 100) / 100))
