...
if laser_type == "cw" or (laser_type == "pulsed"
                          and laser_pulse_duration >= 1e-7):
    print("-----CW-----")
    linear_power_density = old_div(
        laser_output_power,
    ...
    wavelength_adjusted_lidt = linear_power_density * (laser_wavelength /
                                                       float(lidt_wavelength))

    print("[CW] Wavelength adjusted LIDT for your laser:",
          wavelength_adjusted_lidt)
    print("[CW] Specified LIDT for Optic", lidt_maximum_energy_density)

    if wavelength_adjusted_lidt > lidt_maximum_energy_density:
        print(
            "[CW] !---WARNING---! : LIDT threshold specification exceeded for optic "
        )
...
if laser_type == "pulsed":
    ...
    print(
        "[Pulsed] Beam Energy Density (Beam Profile Adjusted) [per Pulse] {} [J/m^2]"
        .format(area_energy_density))
    ...
    adjusted_lidt = area_energy_density * np.sqrt(
        old_div(laser_pulse_duration, lidt_pulse_duration)) * np.sqrt(
            laser_wavelength / float(lidt_wavelength))

    print(
        "[Pulsed] Adjusted (wavelength, pulse duration) LIDT for your laser:",
        adjusted_lidt)
    print("[Pulsed] Specified LIDT for Optic", lidt_maximum_energy_density)
    if adjusted_lidt > lidt_maximum_energy_density:
        print(
            "[Pulsed] !---WARNING---! : LIDT threshold specification exceeded for optic "
        )
