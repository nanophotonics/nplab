...
if laser_type == "pulsed":
	...
	pulse_energy = laser_output_power/laser_pulse_repetition_rate

	print "[Pulsed] Pulse energy: {0} [J]".format(pulse_energy)
	print "[Pulsed] Beam area {0} [m^2]".format(beam_area)
	
	area_energy_density = pulse_energy/beam_area 
	if beam_profile == "gaussian":
	...
	adjusted_lidt = area_energy_density* np.sqrt(laser_pulse_duration/lidt_pulse_duration)*np.sqrt(laser_wavelength/float(lidt_wavelength))
