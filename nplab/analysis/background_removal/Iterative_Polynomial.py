...
def Update_Polynomial(x, y, Mask, Anchors, Anchor_Values):
    ...
    def Loss(Anchor_Values):
        return np.sum(np.abs(
            (Polynomial(x, Anchors, Anchor_Values) - y)[Mask]))
...
def Run(x, y, Poly_Order=4, Auto_Remove=True, Maximum_Iterations=100):
    ...
    while True:
        ...
        Noise_Elements = (np.array(y) - Polynomial(x, Anchors, Anchor_Values))[
            np.array(y) < Polynomial(x, Anchors, Anchor_Values)]
        Noise = np.mean(np.square(Noise_Elements))**0.5

        New_Mask = (np.array(y) <=
                    (Polynomial(x, Anchors, Anchor_Values) + Noise))

        Iteration += 1

        if np.sum(New_Mask == Mask) == len(
                Mask) or Iteration == Maximum_Iterations:
