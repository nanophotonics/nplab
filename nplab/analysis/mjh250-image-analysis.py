...
def get_ellipse_contour(input_image):
    ...
    output_image = (255 * (output_image - np.min(output_image)) /
                    float(np.max(output_image)))
    ...
    output_image, contours, hierarchy = cv2.findContours(
        output_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)

    #find areas of the contours:
    contours_area = [(cnt, cv2.contourArea(cnt)) for cnt in contours]
    areas = [x[1] for x in contours_area]
    largest_contour = [
        cnt for (cnt, area) in contours_area if area == max(areas)
    ][0]
    ...
    for i in range(image_shape[0]):
        for j in range(image_shape[1]):
            if polygonPath.contains_points([[
                    j, i
            ]]):  #ordering of indices is wrong way around to usual, but works!
                mask[i, j] = 1
    return mask
...
def process_image(image, figname):
    ...
    np_only = prod - prod * (
        (get_largest_binary_convex_hull(level_set) - 1) * -1)

    convolved = skimage.feature.canny(np_only, 1, 0.2,
                                      0.7)  #correlate2d(np_only,npmask)
    ...
    def has_valid_segment(image, max_radius=5):
        ...
        for i in range(len(separated)):
            img = separated[i]
            (cx, cy) = CoM_image(img)
            if valid_segment(img, cx, cy,
                             max_radius) and nonzero_pixels(img) > 4:
                ys, xs = draw.circle_perimeter(cy, cx, max_radius)
                for (x, y) in zip(xs, ys):
                    if (x >= 0 and x < img.shape[0]) and (y >= 0 and
                                                          y < img.shape[1]):
                        img[x, y] = 1
                # img[xs,ys] = 1
