...
def Grad(x, y):
	...
	"""
    Output = (np.array(np.array(y).tolist() + [y[-1], y[-2]])) - (
        np.array([y[1], y[0]] + np.array(y).tolist()))
    Output /= (np.array(np.array(x).tolist() + [x[-1], x[-2]])) - (
        np.array([x[1], x[0]] + np.array(x).tolist()))
    return Output[1:-1]
...
def Run(Signal):
    ...
    y = scint.interp1d(Smooth, Score,
                       kind='cubic')(np.linspace(0, np.max(Smooth), 1000))
