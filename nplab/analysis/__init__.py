...
def load_h5(location='.'):
    ...
    path = Path(location)
    return h5py.File(
        path / max(f for f in path.iterdir() if f.suffix == '.h5'), 'r')
...
class Spectrum(np.ndarray):
    ...
    def __array_finalize__(self, obj):
        ...
        if not obj.shape:
            return np.array(obj)
        self.wavelengths = getattr(obj, 'wavelengths',
...
def remove_cosmic_ray(spectrum, thresh=5, smooth=30, max_iterations=10):
    ...
    for i in range(max_iterations):
        ...
        spikes = np.arange(_len)[noise_spectrum > mean_noise +
                                 (thresh * noise_level)]
