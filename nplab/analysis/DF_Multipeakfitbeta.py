...
def plotIntensityRatios(outputFile, plot = True, xBins = 150, yBins = 120, ringFraction = 0.5,
                        closeFigures = False, normalised = True, filterWeird = False):
...
def doStats(outputFile, minBinFactor = 5, sortSpec = True, replaceWhenSorting = False, sortMethod = 'basic', stacks = True,
            hist = True, intensityRatios = True, pointyPeaks = True, doubBools = True, raiseExceptions = False, closeFigures = False):
    ...
    if intensityRatios == True:
        ...
        intensityRatios, cmPeakPositions, irImg = plotIntensityRatios(outputFile, plot = True, xBins = xBins,
                                                                      yBins = yBins, closeFigures = closeFigures, normalised = True,
                                                                      filterWeird = False)
        ...
        intensityRatios, cmPeakPositions, irImg = plotIntensityRatios(outputFile, plot = True, xBins = xBins,
                                                                      yBins = yBins, closeFigures = closeFigures, normalised = False,
                                                                      filterWeird = False)
...
if __name__ == '__main__':
    ...
    if method == 'All':
        ...
        outputFile = sorted([fileName for fileName in os.listdir('.') if
                             fileName.startswith('MultiPeakFitOutput') and (fileName.endswith('.h5') or fileName.endswith('.hdf5'))],
                             key = lambda fileName: os.path.getmtime(fileName))[-1]#Finds most recent outputfile

        with h5py.File(outputFile, 'a') as f:
            doStats(f, minBinFactor = 6, sortSpec = False, replaceWhenSorting = True, sortMethod = 'full', stacks = False, hist = False,
                    intensityRatios = True, pointyPeaks = False, doubBools = False, raiseExceptions = True, closeFigures = True)
