...
for particle_name in particle_names:
    ...
    for item in items:
        if 'Powerseries' in str(item):
            ...
            timescan.calibrate_intensity(R_setup = R_setup_633nm, dark_counts = dark_counts, laser_power = laser_power)
            
            powerseries.append(timescan)
            powerseries_powers.append(laser_power)
#%%
fig, (ax1) = plt.subplots(1, 1, figsize=[16,8])
fig.suptitle('SERS Powerseries (633nm)  \n' + 'Co-TAPP-SMe 80nm NPoM', fontsize='large')
...
for i , timescan in enumerate(powerseries[12:]):
    timescan.integrate_timescan(plot=False)
    spec = timescan.integrated_spectrum
    spec.normalise()
    ax1.plot(timescan.x, spec.y_norm + (i/3), label = str(timescan.dset.attrs['laser_power']) + ' mW')
    #ax1.plot(timescan.x, spec.y + (i*500000), label = str(timescan.dset.attrs['laser_power']) + ' mW')
#plt.ylim(0,0.6e7)
plt.legend()
plt.show()    
