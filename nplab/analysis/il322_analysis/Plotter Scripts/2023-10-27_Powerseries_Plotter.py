...
Plotter for M-TAPP-SMe single scan 633nm powerseries from 2023-10-21_633nm_Powerseries_Track_NPoM_MLAgg.h5
...
Some image plotting to find if focus/stage drift

(samples:
     2023-07-31_Co-TAPP-SMe_80nm_NPoM_b
...
powerseries = []
for key in keys:
    if 'Au_mirror' in key and 'SERS' not in key:
...
#%% Plot single powerseries for single NPoM
...
#%% Same as above but for each particle across scan

'''
Just plotting un-smoothed lowest power spectra to find damage threshold

Take average of un-smoothed lowest power spectra across all particles
''' 

scan_list = ['ParticleScannerScan_0']
avg_powerseries = np.zeros([19, len(spectrum.y)])
#avg_powerseries = list(avg_powerseries)
#avg_powerseries = range(17)
...
for particle_scan in scan_list:
    ...
    for particle in particle_list:
        ...
        save_dir = r'C:\Users\ishaa\OneDrive\Desktop\Offline Data\2023-10-21_NPoM\_'
       # plt.savefig(save_dir + particle_name + '.svg', format = 'svg')
       # plt.close(fig)
        print(particle_name)
        
        
#%% Plot min power spectrum powerseries avg across NPoMs
...
for i in range(0, len(avg_powerseries)):
    ...
    if i == 0:
        ...
        ax.plot(x, avg_powerseries[i], linewidth = 1, color = color, label = powerseries[i-1].laser_power * 1000)
        print('no')
## Label & plot
ax.legend(fontsize = 18, ncol = 5, loc = 'upper center')
ax.get_legend().set_title('Previous laser power ($\mu$W)')
for line in ax.get_legend().get_lines():
    line.set_linewidth(4.0)
fig.suptitle('ParticleScannerScan_0 Average')
ax.set_xlim(250, 1900)
ax.set_ylim(0, avg_powerseries[0].max() * 1.4)
plt.tight_layout(pad = 0.8)

#plt.savefig(save_dir + 'NPoM Avg' + '.svg', format = 'svg')
plt.close(fig)
...
for particle_scan in scan_list:
    ...
    for particle in particle_list:
        ...
        save_dir = r'C:\Users\ishaa\OneDrive\Desktop\Offline Data\2023-10-21_MLAgg\_'
        # plt.savefig(save_dir + particle_name + '.svg', format = 'svg')
        # plt.close(fig)
        print(particle_name)
...
#%% before & after thumb image


particle = my_h5['ParticleScannerScan_1']['Particle_9']

image_before = particle['CWL.thumb_image_0']
image_after = particle['CWL.thumb_image_1']
...
ax1.imshow(image_before, zorder = 500)
ax1.hlines(50, 0, 99, color = 'green', zorder = 600,  linewidth = 2)
ax1.vlines(50, 0, 99, color = 'green', zorder = 600,  linewidth = 2)
ax1.set_title('Before')
ax2.imshow(image_after, zorder = 500)
ax2.hlines(50, 0, 99, color = 'green', zorder = 600,  linewidth = 2)
ax2.vlines(50, 0, 99, color = 'green', zorder = 600,  linewidth = 2)
ax2.set_title('After')
fig.suptitle('NPoM: ParticleScannerScan_0_Particle_9')