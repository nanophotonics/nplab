...
if __name__ == '__main__':
    ...
    if statsOnly == True:
        outputFileName = mpf.findH5File(
            os.getcwd(), nameFormat='MultiPeakFitOutput', mostRecent=True
        )  #finds the most recent file with given name format
        ...
        if extractFirst == True:
            ...
            if pl == True:
                summaryFile = cdf.transferPlSpectra(
                    os.getcwd(),
                    startWl=505,
                    start=startSpec,
                    finish=finishSpec
    ...
    if hours > 0:
        print('\nM8 that took ages. %s hours %s min %s sec' %
              (hours, mins, secs))
