...
#        self._scan_number_changed()
    ...
    def _scan_number_changed(self):
        ...
        try:
            ...
            try:
                g = self._hdf5file["ParticleScannerScan_%d/" %
                                   (self.group_number)]
...
#        self._scan_number_changed()
    ...
    @on_trait_change(
        "use_reference,background_subtract,scan_number,group_number")
    def refresh_data(self):
        ...
        if self.use_reference:
            mask = [
                r < self.reference_threshold for r in (reference - background)
            ]  #mask dodgy pixels
        ...
        self.replot(
            image[old_div(image.shape[0], 2) - 50:old_div(image.shape[0], 2) +
                  50,
                  old_div(image.shape[1], 2) - 50:old_div(image.shape[1], 2) +
                  50, ], zscan, wavelengths, spectrum)
...
def fit_gaussians_to_z_scan(z_scan,
                            z=None,
                            background=None,
                            threshold=0.2,
                            smoothing_width=1.5):
    ...
    if z is None:  #try to find the Z values of the different spectra from metadata if not specified
        ...
        try:
            z = z_scan.attrs.get(
                'dz')  #this should work for datasets in an HDF5 file

        except:
            print(
                "Warning: no valid z positions were found, using indices instead"
            )
    ...
    Particle_array = np.array(
        z_scan, dtype=np.float
    ) - background  #first prepare background-subtracted, thresholded arrays
    ...
    scan_s[
        scan_s <
        0.] = 0.  #smooth and remove negative points so that we don't get NaNs
    ...
    covariance = np.mean((gaussians - mean_x) * (Particle_array - mean_y),
                         axis=0)
...
def extract_all_spectra(datafile, outputfile):
    ...
    for n, group_name in enumerate(group_names):
        ...
        save_group = outputfile.require_group('particleScanSummaries/scan%s' %
                                              n)
        Particle_groups = [
            group for key, group in list(data_group.items())
            if "Particle_" in key
        ]  #filter out scan groups from overview images, etc.

        if len(Particle_groups) > 0:
            shape = (len(Particle_groups), Particle_groups[0]
                     ['alinger.z_scan_0'].attrs.get('wavelengths').shape[0])
            ...
            for name, value in list(
                    Particle_groups[0]['alinger.z_scan_0'].attrs.items()):
                spectra.attrs.create(name, value)

            peak_z = save_group.create_dataset("peak_z", shape)
            standard_deviation = save_group.create_dataset(
                "standard_deviation", shape)
            fitted_background = save_group.create_dataset(
                "fitted_background", shape)
            correlation_coefficient = save_group.create_dataset(
                "correlation_coefficient", shape)

            for i in range(len(Particle_groups)):

                try:
                    spectra[i, :], peak_z[i, :], standard_deviation[
                        i, :], fitted_background[
                            i, :], correlation_coefficient[
                                i, :] = fit_gaussians_to_z_scan(
                                    data_group['Particle_%d/alinger.z_scan_0' %
                                               i])

                except Exception as e:
                    print("Failed to get alinger.z_scan_0 in group %s." %
                          group_name)
                    raise e
...
if __name__ == "__main__":
    ...
    for filename in os.listdir('.'):
        ...
        if re.match(
                '\d\d\d\d-[01]\d-[0123]\d.h5', filename
        ):  #Finds first instance of file with name in "yyyy-mm-dd.h5" format
            break
