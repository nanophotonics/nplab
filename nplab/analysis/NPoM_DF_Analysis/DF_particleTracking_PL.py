...
wizard = TrackingWizard(CWL,
                        equipment_dict,
                        task_list=['CWL.thumb_image', 'alinger.z_scan'])
...
def df_pl_with_laser_scan(power_iter=1,
    ...
    """
    irad_group = wizard.particle_group.create_group(
        'dark field with irradiation')

    log = wizard.data_file['nplab_log']
    lastEventName = sorted(
        list(log.keys()),
    ...
    for i in range(power_iter):
        for i in range(
                number_of_iter
        ):  #while stop_event.is_set()==False and iterations<spectra_num:
            ...
            print(laser_power)
            for i in range(number_of_spec):
                dIrad = irad_group.create_dataset(name='PL_%s_%s' %
                                                  (laser_power, i),
                                                  data=spec.read_spectrum(),
                                                  attrs={
                                                      'laser_power':
