...
class VirtualDimer:
    ...
    def initialize_constants(self):
        ...
        self.plasma_freq = 2 * np.pi * 2.99792458e8 / (
            self.plasma_wl * 1e-9)  #radial frequency, Constant for Au
        self.eps_0 = pow((pow(2.9979e8, 2) * 4 * np.pi * 1e-7),
                         -1)  #Constant in F/m (SI units)
        self.cond_quant = 7.74809173e-5  #Constant

    def calc_chi(self):
        self.chi = 1.0242 + self.np_size * 0.012785 - 0.0001375 * pow(
            self.np_size, 2)
    ...
    def calc_eps_inf(self):
        ...
        self.eps_inf = 9.38 - 0.00339 * self.np_size + 0.00021 * pow(
            self.np_size, 2)
    ...
    def calc_cavity_wl(self):  # calculates energies of lowest mode
        ...
        en = wp / np.sqrt(
            ...
            (self.gap_size * 4.2)
        )  # alpha antinodes: in NPoMv2 {4.2,9.4,15} to fit better?

        self.cavity_wl = self.wl_to_ev(en)

    def calc_antenna_wl(
        self
    ):  #calculates coupled mode for spherical dimer using Felix Benz's circuit model
        wd = 1 / np.sqrt(4 * self.eta * self.gap_ri + 2 * self.gap_ri +
                         self.eps_inf)
        ...
        if self.conductance == 0:
            ...
            Cs = 2 * np.pi * (self.np_size /
                              2) * self.eps_0 * self.gap_ri * 1e-9
    ...
    def calc_mixing(
        self
    ):  # calculates coupled plasmon from cavity mode coupled to antenna mode
        ...
        eo = 0.5 * (e1 + e2) - np.sqrt(
            0.25 * (e2 - e1)**2 +
            0.11**2)  # V=0.09 in NPoMv2 for 5 modes, increase to match
