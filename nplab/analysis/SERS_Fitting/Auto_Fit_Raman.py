...
def Attempt_To_Fit(Shift,
    ...
    while Number_Of_Peaks > 0:
        ...
        if len(Parameters) > 0:
            Loss = []
            for i in Parameters:
                Loss.append(np.sum(np.abs(Array -
                                          Multi_L_Constant(Shift, *i))))
...
def Run(Input, Shift, Width=10, Smoothing_Factor=5, Noise_Threshold=2):
    ...
    for i in Maxima:
        ...
        if H >= Threshold:
            ...
            Peak_Shifts.append(((Shift[int(i) + 1] - Shift[int(i)]) * i % 1) +
                               Shift[int(i)])
    First_Draft = Attempt_To_Fit(Shift, Smooth, Peak_Shifts, Peak_Heights,
                                 Width, Threshold)
