...
def Fit(Shift,
    ...
    Smooth = ndimf.gaussian_filter(
        Signal, old_div(Smoothing_Width, np.median(np.abs(np.diff(Shift)))))
    ...
    x = np.linspace(
        ...
        int(
            np.round(
                old_div((np.max(Shift) - np.min(Shift)),
                        np.min(np.abs(np.diff(Shift)))))) + 1)
    ...
    x2 = np.linspace(
        ...
        int(
            np.round(
                old_div((np.max(Shift) - np.min(Shift)),
                        np.min(np.abs(np.diff(Shift)))))) + 1)
    y2 = scintp.interp1d(x, Grad, kind='cubic')(x2) - scintp.interp1d(
        x, Grad, kind='cubic')(-x2)
    ...
    Edges = np.round(np.linspace(Position,
                                 len(Smooth) - 1,
                                 Poly_Order_S + 2)).astype(int)
    ...
    def Generate(Vector):
        ...
        Laser_Leak = Vector[7 + Poly_Order_AS + Poly_Order_S:10 +
                            Poly_Order_AS + Poly_Order_S]

        Output = Shift * 0
        Output += Polynomial(Shift, AS_Anchors, Poly_AS) * (Shift < 0)
        Output += Polynomial(Shift, S_Anchors, Poly_S) * (Shift >= 0)

        Mask = Sigmoid(Shift, *Sigmoid_Data_AS) + Sigmoid(
            Shift, *Sigmoid_Data_S)
    ...
    Vector[-3] = np.max(
        Smooth[np.argmin(np.abs(Shift + 10)):np.argmin(np.abs(Shift -
                                                              10))]) - Constant
    ...
    for Iteration in range(Iterations):
        ...
        while End is False:
            ...
            for i in range(Skip):
                Possible_Peak.append(False)
            for i in range(len(Above_Std))[Skip:-Skip]:
                if np.sum(Above_Std[i - Skip:i - Skip + Peak_Threshold_Window]
                          ) == Peak_Threshold_Window:
                    Possible_Peak.append(True)
                else:
                    Possible_Peak.append(False)
            for i in range(Skip):
            ...
            if np.sum(Possible_Peak) > 0:
                ...
                for i in np.array(list(range(len(Shift))))[Possible_Peak]:
                    Loss.append(
                        np.sum(
                            np.abs(
                                L(Shift, np.max([Sub[i], 0]), Shift[i],
                                  Default_Peak_Width) - Sub)))
                    Checked.append(i)

                Peaks_Vector += [
                    Sub[Checked[np.argmin(Loss)]],
                    Shift[Checked[np.argmin(Loss)]], Default_Peak_Width
                ]
                Bounds += [(0, np.inf), (-np.inf, np.inf),
                           (Minimum_Peak_Width, Maximum_Peak_Width)]
                ...
                Peaks_Vector = spo.minimize(Sub_Loss,
                                            Peaks_Vector,
        ...
        def Full_Generate(x, *Input):
            ...
            Laser_Leak = Input[7 + Poly_Order_AS + Poly_Order_S:10 +
                               Poly_Order_AS + Poly_Order_S]
        ...
        while End is False:
            ...
            if np.min(Loss) < Current_Loss:
                Arg = np.argmin(Loss)
                Peaks_Vector = Peaks_Vector[:Arg * 3] + Peaks_Vector[Arg * 3 +
                                                                     3:]
                Bounds[0] = Bounds[:-3]
                Bounds[1] = Bounds[:-3]
        ...
        for i in range(old_div(len(Peaks_Vector), 3)):
            ...
            Fraction = old_div(scint.simps(Peak * Mask, Shift),
                               scint.simps(Peak, Shift))
    ...
    No_Background_Fit = Full_Generate(
        None, *(Vector + Peaks_Vector)) - Generate(Vector)
    ...
    return np.array([Fit, Background_Removed,
                     No_Background_Fit]), Peak_Dictionary
