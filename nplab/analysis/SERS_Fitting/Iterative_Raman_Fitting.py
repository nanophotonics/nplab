...
def Wavelet_Estimate_Width(x_axis,
                           Signal,
                           Maximum_Width,
                           Smooth_Loss_Function=2):
...
def Add_New_Peak(x_axis,
    ...
    for i in range(Regions):
        Bounds = [(0, np.inf),
                  (i * Sections + Start, (i + 1) * Sections + Start),
                  (0, Maximum_Width)]
...
def Optimise_Prexisting_Peaks(x_axis,
                              Signal,
                              Peak_Params,
                              Max_Width,
                              Peak_Type='L'):
    ...
    for i in range(len(Heights)):
        ...
        Bounds += [(0, np.inf), (np.min(x_axis), np.max(x_axis)),
                   (0, Max_Width)]
...
def Run(x_axis,
    ...
    while Regions <= len(Signal):
        ...
        if Initial_Fit is None:
            ...
            Output = Optimise_Prexisting_Peaks(x_axis, Signal, Initial_Fit,
                                               Max_Width, Peak_Type)
            Initial_Fit = None
        ...
        if len(Loss_Results) > 1:
            if Loss_Results[-1] >= Loss_Results[-2]:  #Has loss gone up?
                ...
                for i in Old:
                    ...
                    Temp.append([
                        Current[0][Arg], Current[1][Arg]
                    ])  #For each old peak, find the closest new peak
                ...
                if True not in (Seperation >= Threshold).tolist(
                ):  #Check if all peaks have moved by less than 0.25 FWHM
                    Regions *= 5
...
def Worker_Function(x_axis, Signal, Maximum_FWHM, Regions,
                    Minimum_Width_Factor, Peak_Type, Initial_Fit, Number):
    Fit = Run(x_axis,
              Signal,
              Maximum_FWHM,
              Regions,
    ...
    for i in range(len(Signals)):
        Processes.append(
            Pool.apply_async(Worker_Function,
                             args=(x_axis, Signals[i], Maximum_FWHM, Regions,
                                   Minimum_Width_Factor, Peak_Type,
                                   Initial_Peaks, i)))
