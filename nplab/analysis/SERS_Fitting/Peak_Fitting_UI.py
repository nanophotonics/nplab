...
def Select_Time_Range(Min, Max):
    ...
    while Continue is False:
        ...
        while Lower is None:
            Input = eval(
                input(
                    'Please Enter a Lower Spectrum Number (Type \'Cancel\' to cancel): '
                ))
        ...
        while Upper is None:
            Input = eval(
                input(
                    'Please Enter a Upper Spectrum Number (Type \'Cancel\' to cancel): '
                ))
            if str(Input).upper() == 'CANCEL':
                return None, None
            else:
        ...
        Input = eval(
            input('Continue with range ' + str(Lower) + ' to ' + str(Upper) +
                  '? (Y/N): '))
...
def Input_Core_Number():
    ...
    while Output is None:
        Input = eval(
            input('Please Enter the Number of CPU Cores to Utilise: '))
        try:
            Input = int(Input)
            if Input <= 0:
    ...
    for i in range(len(Array)):
        ...
        Processes.append(
            Pool.apply_async(Fitting_Worker,
                             args=(Constant_plus_Lorentzians, x_axis, Array[i],
                                   Initial, (Bounds_Lower, Bounds_Upper), i)))
...
def Run(Array, x_axis, Threshold=None, colormap='inferno'):
    ...
    print('==========')
    print(
        "Fitting will be faster and less likely to fail if each spectrum is split into sections."
    )
    print(
    ...
    for i in Sections:
        ...
        Results.append(
            Run_Fitting(
                np.transpose(np.transpose(To_Fit)[int(i[0]):int(i[1])]),
                ...
                while Pos1 < len(Sorting_Order):
                    ...
                    while Pos2 < len(Sorting_Order):
                        ...
                        if Sorting_Positions[Sorting_Order[Pos1]] is not None:
                            Change += abs(Current[Pos1] - Sorting_Positions[
                                Sorting_Order[Pos1]]) - abs(
                                    Current[Pos2] -
                                    Sorting_Positions[Sorting_Order[Pos1]])
                        if Sorting_Positions[Sorting_Order[Pos2]] is not None:
                            Change += abs(Current[Pos2] - Sorting_Positions[
                                Sorting_Order[Pos2]]) - abs(
                                    Current[Pos1] -
