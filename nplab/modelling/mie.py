...
def calculate_pi_tau(mu, n_max):
    ...
    for n in range(2, n_max):
        pi_n[n] = ((2.0 * n - 1) /
                   (n - 1)) * mu * pi_n[n - 1] - ((n) / (n - 1)) * pi_n[n - 2]
...
def mie_S1_S2(m, x, mus, n_max):
    ...
    for mu in mus:
        ...
        for n in range(n_max):
            ...
            S1 = S1 + (float(2.0 * N + 1) /
                       (N**2 + N)) * (a[n] * pi[n] + b[n] * tau[n])
            S2 = S2 + (float(2.0 * N + 1) /
                       (N**2 + N)) * (b[n] * pi[n] + a[n] * tau[n])
...
def main3():
    ...
    for r in rs:
        ...
        ax1.legend()
        plt.title("Radius [nm]: {0}, $\lambda$: {1}".format(
            r / 1e-9, lambda_max / 1e-9))
...
def main():
    ...
    for r in np.linspace(1e-9, 4e-7, 50):
        ...
        plt.title("Particle radius [nm]:{0},x:{1},\nm:{2}, n_max:{3}".format(
            r / 1e-9, x, m, n_max))
        # plt.show()
        plt.savefig("C:\\Users\im354\Pictures\Mie\particle_{}.png".format(
            r / 1e-9))
...
def main4():
    wavelength_range = np.asarray(
        [1e-9 * wl for wl in np.linspace(450, 1000, 550)])
    ...
    plt.title(
        "Normalized scattering cross section\n Normalization: z/z_min, z_min = {}"
        .format(zmin))

    ratio_low = scattering_cross_section(
        radius=120e-9, wavelength=450e-9) / scattering_cross_section(
            radius=100e-9, wavelength=450e-9)
    ratio_high = scattering_cross_section(
        radius=120e-9, wavelength=580e-9) / scattering_cross_section(
            radius=100e-9, wavelength=580e-9)
