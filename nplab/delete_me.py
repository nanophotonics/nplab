    ...
    for key, value in list(dict_of_attributes.items()):
        if value is not None:
            try:
                ...
                print(
                    "Warning, metadata {0}='{1}' can't be saved in HDF5.  Saving with str()"
                    .format(key, value))
                attrs[key] = str(value)
...
def h5_item_number(group_or_dataset):
    ...
    m = re.search(
        r"(\d+)$",
        group_or_dataset.name)  # match numbers at the end of the name
...
def get_unique_filename(data_dir, basename='data', fformat='.h5'):
    ...
    file_path = os.path.join(output_dir,
                             basename + '_' + str(unique_id) + fformat)
...
def sort_by_timestamp(hdf5_group):
    ...
    try:
        ...
        for value in list(hdf5_group.values()):
            ...
            try:
                time_stamp_float = datetime.datetime.strptime(
...
class Group(h5py.Group, ShowGUIMixin):
    ...
    def numbered_items(self, name):
        ...
        """
        items = [
            wrap_h5py_item(v) for k, v in list(self.items())
            if k.startswith(name)  # only items that start with `name`
            and re.match(r"_*(\d+)$", k[len(name):])
        ]  # and end with numbers
    ...
    def create_group(self,
        ...
        if auto_increment and name is not None:
            name = self.find_unique_name(
                name)  #name is None if creating via the dict interface
        g = super(Group, self).create_group(name)
        if timestamp:
            g.attrs.create('creation_timestamp',
    ...
    def require_group(self, name):
        ...
        return Group(super(
            Group, self).require_group(name).id)  # wrap the returned group
    ...
    def create_dataset(self,
        ...
        dset = super(Group, self).create_dataset(name, shape, dtype, data,
                                                 *args, **kwargs)
        if timestamp:
            dset.attrs.create('creation_timestamp',
                              datetime.datetime.now().isoformat().encode())
        if hasattr(data,
                   "attrs"):  #if we have an ArrayWithAttrs, use the attrs!
    ...
    def require_dataset(self,
        ...
        if name not in self:
            dset = self.create_dataset(name, auto_increment, shape, dtype,
                                       data, attrs, timestamp, *args, **kwargs)
        else:
            dset = self[name]
        return dset
    ...
    def require_resizable_dataset(self,
        ...
        if name not in self:
            dset = self.create_resizable_dataset(name, shape, maxshape,
                                                 auto_increment, dtype, attrs,
    ...
    def __init__(self,
        ...
        if save_version_info and self.file.mode != 'r':
            ...
            try:
                self.attrs.create(
                    "version_info_%04d" % n,
                    np.string_(nplab.utils.version.version_info_string()))
            except:
    ...
    if _current_datafile is None and create_if_none:
        ...
        try:  # we try to pop up a Qt file dialog
            ...
            if not isinstance(fname, str):
                fname = fname[
                    0]  # work around version-dependent Qt behaviour :(
            if len(fname) > 0:
                print(fname)
                if not "." in fname:
                    fname += ".h5"
...
def set_current(datafile, **kwargs):
    ...
    if isinstance(datafile, DataFile):
        ...
        try:
            _current_datafile = DataFile(datafile,
                                         **kwargs)  # open a new datafile
            return _current_datafile
        except Exception as e:
            print("problem opening file")
            if os.path.getsize(datafile) < 100:  #1kB/10
                os.remove(
                    datafile)  # dirty hack to work around mode=a not working
...
def set_temporary_current_datafile():
    ...
    print(
        "WARNING: using a file in memory as the current datafile.  DATA WILL NOT BE SAVED."
    )
...
def set_current_group(selected_object):
    ...
    try:
        ...
        if type(selected_object) == Group or type(
                selected_object) == h5py.Group:
