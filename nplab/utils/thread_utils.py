...
def locked_action_decorator(wait_for_lock=True):
    ...
    def decorator(function):
        ...
        def locked_action(self, *args, **kwargs):
            ...
            try:
                if wait_for_lock:
                    self._nplab_action_lock.acquire(
        ...
        def background_action(self, *args, **kwargs):
            if background_by_default:
                ...
                def join_and_return_result(
                    self
                ):  #this gets added to a thread so we can extract the return value
                    self.join()
                    return self.returned_value

                t.join_and_return_result = functools.partial(
                    join_and_return_result, t)  #add method to the thread
                return t
            else:
                return function(self, *args, **kwargs)
...
backgroundable_action = background_action_decorator(
