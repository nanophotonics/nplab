...
def Convert_Pyqt_to_qtpy(path, avoid_files=None):
    ...
    file_locations = [
        os.path.join(dirpath,
                     f)  #Create a list of file paths from the top level path
    ...
    for filename in file_locations:
        ...
        for line in fileinput.input(filename, inplace=True):
            if ('import ' in line) and (('QtCore' in line) or
                                        ('QtGui' in line) or ('uic' in line)):
                if '*' in line:
                    nplab_import_line = line.replace('PyQt4',
                                                     'nplab.utils.gui')
            ...
            for command in line.split(' '):
                ...
                if 'QtCore.' in command:
                    ...
                    try:
                        func = re.search(r"QtCore\.([0-9a-zA-Z_]+)",
                                         command).group(1)
                    except IndexError:
                        continue
                    if func == 'pyqtSignal':
