...
def jpeg_encode(image, quality=90):
    ...
    """
    ret, encoded_array = cv2.imencode(
        '.jpeg', image, (cv2.cv.CV_IMWRITE_JPEG_QUALITY, quality))
...
def png_encode(image, compression=3):
    ...
    """
    ret, encoded_array = cv2.imencode(
        '.png', image, (cv2.cv.CV_IMWRITE_PNG_COMPRESSION, compression))
    assert ret, "Error encoding image"
...
def test_image_codecs():
    ...
    png = png_encode(img)
    print("PNG size is %d (%.d%%)" %
          (png.size, old_div((100 * png.size), img.size)))
    assert png.size < img.size, "The PNG was bigger than the image!"

    jpeg = jpeg_encode(img)
    print("JPEG size is %d (%d%%)" %
          (jpeg.size, old_div((100 * jpeg.size), img.size)))

    assert jpeg.size < img.size, "The JPEG was bigger than the image!"

    assert np.all(
        png_decode(png) == img), "The PNG did not uncompress losslessly."
    assert np.any(
        jpeg_decode(jpeg) != img), "The JPEG uncompressed losslessly - wtf?"
