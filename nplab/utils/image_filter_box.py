...
class Camera_filter_Control_ui(QuickControlBox):
    ...
    def __init__(self, filter_box):
        super(Camera_filter_Control_ui,
              self).__init__(title='Camera_filter_Controls')
        ...
        self.add_combobox('current_filter_index',
                          options=self.filter_box.filter_options)
...
def strided_rescale(g, bin_fac=4):
    ...
    try:
        strided = as_strided(
            g,
            shape=(g.shape[0] // bin_fac, g.shape[1] // bin_fac, bin_fac,
                   bin_fac),
            strides=((g.strides[0] * bin_fac, g.strides[1] * bin_fac) +
                     g.strides))
        strided = strided.sum(axis=-1).sum(axis=-1)
        strided = np.uint8((strided - np.min(strided)) * 254.0 /
                           (np.max(strided) - np.min(strided)))
...
def StrBiThresOpen(g,
    ...
    try:
        ...
        strided = cv2.bilateralFilter(np.uint8(strided), bilat_size,
                                      bilat_size, 50)  # noise reduction
        #    strided[strided<threshold]=1
        #  strided = cv2.adaptiveThreshold(strided,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
        #       cv2.THRESH_BINARY,3,2)
        strided = cv2.adaptiveThreshold(strided, 255,
                                        cv2.ADAPTIVE_THRESH_MEAN_C,
                                        cv2.THRESH_BINARY, 101, -1 * threshold)
...
def STBOC_with_size_filter(g,
                           ...
                           return_centers_and_radii=False):
    g = np.copy(g)
    strided = StrBiThresOpen(g, bin_fac, threshold, bilat_size, bilat_height,
                             morph_kernel_size)
    contours, hierarchy = cv2.findContours(strided, cv2.RETR_TREE,
    ...
    if return_centers_and_radii:
        ...
        for cnt, radius in zip(centers, radii):
            cv2.circle(g, cnt, int(radius * 2), (255, 0, 0),
                       2)  # image with a red circle
        return g
    elif show_particles:
...
def find_particles(self, img=None, border_pixels=50):
    ...
    return [
        np.array(p) + 15 for p in ndimage.measurements.center_of_mass(
            img, labels, list(range(1, nlabels + 1)))
