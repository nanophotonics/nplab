...
def plot_skewed_image(image, corners, axes=None, *args, **kwargs):
    ...
    assert len(shape) == 2 or (
        len(shape) == 3 and (shape[2] == 3 or shape[2] == 4)
    ...
    assert np.array(corners).shape == (
        4, 2
    ), "The 'corners' argument must be four two-element positions, i.e. have shape (4, 2)"
    ...
    if len(shape) == 2:  #grayscale image
        mesh = ax.pcolormesh(coordinate_array[:, :, 0],
                             coordinate_array[:, :, 1], image, *args**kwargs)
    else:
        ...
        mesh = ax.pcolormesh(coordinate_array[:, :, 0], coordinate_array[:, :,
                                                                         1],
                             image[:, :, 0], *args, **kwargs)
        ...
        mesh.set_color(
            old_div(image.reshape((shape[0] * shape[1], shape[2])), divisor))
