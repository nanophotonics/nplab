...
def popup_widget(widget):  # TODO: what is "widget"?
    if widget.isVisible(
    ):  # doesn't need to be created and shown just brought to the front
        pass
    else:
        ui = widget()
        ui.show()
...
def show_guis(instruments, block=True):
    ...
    app = get_qt_app()
    uis = [
        i.show_gui(blocking=False) for i in instruments
        if hasattr(i, "get_qt_ui")
    ]
    # DataBrowserImprovements swapped get_qt_ui for show_gui(block=False)
    traits = [
        i.edit_traits() for i in instruments if hasattr(i, "edit_traits")
    ]
    for ui in uis:
        ui.show()
    if block:
