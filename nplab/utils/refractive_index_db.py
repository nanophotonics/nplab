...
class RefractiveIndexInfoDatabase(object):
    def __init__(self):
        ...
        library_path = os.path.normpath(dirpath +
                                        "/refractive_index_db_lib.yml")
        #print library_path
    ...
    def refractive_index_generator(cls, label):
        ...
        def generator(required_wavelength, scale="nm", debug=0):
            ...
            if required_wavelength < min_wl:
                raise ValueError(
                    "Required wavelength: {0} below minimum in dataset: {1}".
                    format(required_wavelength, min_wl))

            elif required_wavelength > max_wl:
                raise ValueError(
                    "Required wavelength: {0} above maximum in dataset: {1}".
                    format(required_wavelength, max_wl))
